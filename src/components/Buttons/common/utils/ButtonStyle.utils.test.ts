...
describe('Running tests for ButtonStyle utils', () => {
    ...
    describe('Running tests for background color of solid buttons', () => {
        ...
        test('Given no theme-resolver, when getInitialBackgroundColor is called, background color is the default one', () => {
            expect(getInitialBackgroundColor(props)).toBe(
                DefaultButtonThemes.PRIMARY.backgroundColor
            );
        });

        test('Given a theme-resolver, when getInitialBackgroundColor is called, background color is the default color for that theme-resolver', () => {
            expect(
                getInitialBackgroundColor({
                    ...props,
        ...
        test('Given disabled prop and no theme-resolver, when getInitialBackgroundColor is called, background color is the default disabled color', () => {
            expect(
                getInitialBackgroundColor({ ...props, disabled: true })
            ).toBe(DefaultButtonThemes.PRIMARY.disabledBackgroundColor);
        });

        test('Given disabled prop and a theme-resolver, when getInitialBackgroundColor is called, background color is the disabled color for that theme-resolver', () => {
            expect(
                getInitialBackgroundColor({
                    ...props,
                    disabled: true,
    ...
    describe('Running tests for border color', () => {
        test('Given no theme-resolver, when getInitialBorderColor is called, border color is the default one', () => {
            expect(getInitialBorderColor({})).toBe(
                DefaultButtonThemes.PRIMARY.borderColor
            );
        });

        test('Given a theme-resolver, when getInitialBorderColor is called, border color is the default color for that theme-resolver', () => {
            expect(
                getInitialBorderColor({
                    buttonTheme: DefaultButtonThemes.SECONDARY,
                })
        ...
        test('Given disabled prop and no theme-resolver, when getInitialBorderColor is called, border color is the default disabled color', () => {
            expect(getInitialBorderColor({ disabled: true })).toBe(
                DefaultButtonThemes.PRIMARY.disabledBorderColor
            );
        });

        test('Given disabled prop and a theme-resolver, when getInitialBorderColor is called, border color is the disabled color for that theme-resolver', () => {
            expect(
                getInitialBorderColor({
                    disabled: true,
                    buttonTheme: DefaultButtonThemes.SECONDARY,
    ...
    describe('Running tests for text color of solid buttons', () => {
        ...
        test('Given no theme-resolver, when getInitialTextColor is called, text color is the default content color', () => {
            expect(getInitialTextColor(props)).toBe(
                DefaultButtonThemes.PRIMARY.contentColor
            );
        });

        test('Given a theme-resolver, when getInitialTextColor is called, text color is the default content color for that theme-resolver', () => {
            expect(
                getInitialTextColor({
                    ...props,
                    buttonTheme: DefaultButtonThemes.SECONDARY,
        ...
        test('Given disabled prop and no theme-resolver, when getInitialTextColor is called, text color is the default disabled content color', () => {
            expect(getInitialTextColor({ ...props, disabled: true })).toBe(
                DefaultButtonThemes.PRIMARY.disabledContentColor
            );
        });

        test('Given disabled prop and a theme-resolver, when getInitialTextColor is called, text color is the default disabled content color for that theme-resolver', () => {
            expect(
                getInitialTextColor({
                    ...props,
                    disabled: true,
